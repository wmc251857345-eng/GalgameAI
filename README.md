...
| ⭐ | **我的评分与笔记** | 每款游戏 1~5 星个人评分（区别于外部评分）+ 私人笔记，评分可排序，笔记可搜索 |
| 🎯 | **想玩清单** | 还没入库的种草记录：看到想玩的先记下来，支持备注；VNDB 作品弹层里一键加入 |
| ☑️ | **批量操作** | 库页多选游戏：批量设游玩进度、批量加标签、批量删除，几十款游戏几秒整理完 |
| 🎮 | **一键启动** | 自动识别主程序，支持启动参数，自动记录游玩时长（重启后也能补记） |
| 💾 | **存档快照** | **关游戏自动备份存档**，每个游戏保留 20 个历史版本，随时一键回滚；支持导入他人分享的存档 |
| 🔁 | **多 AI 轮询** | 配置多个 AI 服务商，一个限速/挂了自动切换，不怕 AI 掉线 |
| 🛡️ | **稳定可靠** | 自动备份数据库、防多开、防误删（升级前先备份旧数据）、全程中文日志可查、内置更新检查 |
...
**全新功能**
- ⭐ **我的评分**：详情页点星星打 1~5 分（再点一下清除），卡片/列表显示星标，库页新增「我的评分」排序——自己的分和 VNDB 外部分分开看
- 📝 **个人笔记**：每款游戏一块私人笔记（编辑页可写），攻略进度/汉化组/感想随手记，全局搜索能搜到
- 🎯 **想玩清单**：侧栏新增「想玩清单」——还没入库的种草游戏先记下来，支持备注；厂商墙的作品弹层里可一键「加入想玩」
- ☑️ **批量操作**：库页新增「批量」模式，多选后一键设游玩进度（进行中/通关/未开始）、批量加标签、批量删除
- 🔄 **更新检查**：启动时静默检查 GitHub Releases（24h 缓存），有新版侧栏出现提示条；设置页可手动检查 + 查看升级方法
- 🔍 **搜索增强**：搜索现在覆盖简介和笔记（此前只有标题/厂商/标签）

**修复的问题**
- 🩸 **误启动幽灵时长**：秒开秒关的误启动会留下未结算会话，下次启动被按"发现时刻"补记，最长凭空多出 **24 小时**游玩时长——现在误启动立即按 0 秒结算
- 💾 **升级不再重建数据库**：旧版本号变化时会清空重建库（虽有备份但要手动恢复）；v1.1 起全部走无损迁移（只加列不动数据），老库原样保留
- 🗑 删除游戏现在同步清理存档备份元数据（不再残留孤儿记录）
- ☁ 配置存档路径不再清零该游戏的备份次数/大小统计（INSERT OR REPLACE → UPSERT）
- 🛡 游戏运行中禁止恢复存档（防止删到正在使用的文件）；备份目标指向未接入的 U 盘/移动盘不再报错崩溃
- ✂ 启动参数含多个参数时正确拆分传递（旧版整串被当成一个带引号的参数）
- 🖨 二次裁剪封面不再裁错区域（改用裁剪前原图做选框底图）
- 🧊 一批 UI 卡死防御：扫描进度轮询、翻译状态轮询、设置页加载等任一桥接调用失败不再永久卡"转圈"/禁用按钮；约 40 个前端桥接调用补齐超时保护
- 🔗 厂商档案里点击已拥有的作品、作品弹层"打开本地游戏"按钮失效（引用了不存在的方法）已修复
- 🧹 清理：备份元数据死代码（引用不存在的列）、测试脚本硬编码旧机器路径

**移除**
- ❌ **Locale Emulator 集成**：实践验证不可靠（LEProc 只是启动器、秒退且依赖每游戏配置文件，启动成功率与时长统计均无保障），v1.1 起移除。需要转区的用户建议直接用 Locale Emulator 右键菜单启动游戏，GALA 照常记录时长。
...
**Q：游戏启动乱码？**
v1.1 起已移除内置 Locale Emulator 开关（实践验证不可靠）。需要转区的老游戏：直接用 Locale Emulator 的资源管理器右键菜单启动即可，不影响 GALA 的时长统计与存档备份。
